...
local function promptHandler(parent)
    ...
    if prompt.ActionText == "End Shift" then
        ...
        while baristaEnabled and (os.clock()-t0) < 6 do
            if not prompt.Parent then prompt = parent:FindFirstChildWhichIsA("ProximityPrompt") end
...
local function dlog(msg)
    ...
    if #debugLog > MAX_LOG then table.remove(debugLog) end
end
...
task.spawn(function()
    while task.wait(0.1) do
        ...
        if not char or not hrp or not hum or hum.Health <= 0 then
            dlog("karakter tidak valid, nunggu...")
            task.wait(0.5); continue
        end

        -- Team check
        if not LocalPlayer.Team or LocalPlayer.Team.Name ~= "Barista" then
            dlog("Team: " .. (LocalPlayer.Team and LocalPlayer.Team.Name or "NIL") .. " — fire TeamChangeRequest")
            WindUI:Notify({ Title="Barista", Content="Masuk job Barista...", Duration=3, Icon="coffee" })

            local oldChar = LocalPlayer.Character
            local jev = ReplicatedStorage:FindFirstChild("JobEvents")
            local tcr = jev and jev:FindFirstChild("TeamChangeRequest")

            if tcr then
                tcr:FireServer("Barista", 11378976, 1, 0, "Detector")
                dlog("TeamChangeRequest fired")
            else
                dlog("ERROR: JobEvents/TeamChangeRequest TIDAK ADA")
            ...
            local newHRP = getHRP()
            if newHRP then
                dlog("Karakter spawn — seatTeleport ke START_POS")
                seatTeleport(CFrame.new(START_POS))
                task.wait(0.5)
            else
                dlog("ERROR: HRP tidak muncul setelah team change")
            end
            continue
        end
        ...
        if not job then
            dlog("ERROR: workspace.BaristaJob TIDAK ADA")
            task.wait(1); continue
        end
        if not interactions then
            dlog("ERROR: BaristaJob.Interactions TIDAK ADA")
            task.wait(1); continue
        end
        ...
        -- Log path findings sekali
        if not startPart then
            dlog("StartPart TIDAK ADA — interactions children: " .. (function()
                local s = ""
                for _, c in ipairs(interactions:GetChildren()) do s = s .. c.Name .. " " end
                return s
            end)())
        end
        ...
        dlog("Team:OK | distToDrop:" .. math.floor(distToDrop)
            .. " | working:" .. tostring(baristaWorking)
            .. " | canStart:" .. tostring(canStart)
            .. " | supply:" .. tostring(supplyPrompt and supplyPrompt.Enabled)
            .. " | machine:" .. tostring(machinePrompt and machinePrompt.Enabled)
            .. " | register:" .. tostring(registerPrompt and registerPrompt.Enabled))

        if not baristaWorking or distToDrop > 80 or canStart then
            if distToDrop > 80 then
                dlog("Terlalu jauh — seatTeleport ke START_POS")
                seatTeleport(CFrame.new(START_POS))
                task.wait(0.5)
            elseif distToStart > 6 then
                dlog("baristaTeleport ke START_POS")
                baristaTeleport(CFrame.new(START_POS))
                task.wait(0.3)
            end

            dlog("promptHandler START_POS")
            local ok = promptHandler(startPart)
            dlog("promptHandler result: " .. tostring(ok))
            if not ok then task.wait(0.5); continue end

            baristaWorking = true
            baristaLastAct = os.clock()
            dlog("baristaTeleport ke BREW_POS")
        ...
        if supplyPrompt and supplyPrompt.Enabled then
            dlog("SUPPLY — teleport ke supply")
            baristaLastAct = os.clock()
        ...
        if distIdle > 3.5 then
            dlog("IDLE drift — baristaTeleport ke BREW_POS")
            baristaTeleport(CFrame.new(BREW_POS))
        end
        ...
        if (os.clock()-baristaLastAct) > 18 then
            ...
            WindUI:Notify({ Title="Barista", Content="Stuck #"..baristaStuck.." — refresh shift", Duration=3, Icon="coffee" })
            baristaTeleport(CFrame.new(START_POS))
            task.wait(0.3)
...
-- ── Scan workspace sekali saat script load ────────────────────
task.spawn(function()
    task.wait(3)
    dlog("=== SCAN AWAL ===")
    ...
    dlog("BaristaJob   : " .. (job and "ADA" or "TIDAK ADA"))
    dlog("JobEvents    : " .. (jev and "ADA" or "TIDAK ADA"))
    dlog("TeamChangeReq: " .. (tcr and "ADA" or "TIDAK ADA"))

    if job then
        local inter = job:FindFirstChild("Interactions")
        dlog("Interactions : " .. (inter and "ADA" or "TIDAK ADA"))
        if inter then
            local names = ""
            for _, c in ipairs(inter:GetChildren()) do names = names .. c.Name .. " " end
            dlog("Inter children: " .. names)
        end
    end

    -- Cari semua ProximityPrompt yang ada kata Shift
    for _, c in ipairs(workspace:GetDescendants()) do
        if c:IsA("ProximityPrompt") then
            local at = c.ActionText or ""
            if at:lower():find("shift") or at:lower():find("barista") then
                dlog("PROMPT FOUND: " .. c:GetFullName() .. " | Text: " .. at)
            end
        end
    end
...
farmSection:Toggle({
    ...
    Callback = function(state)
        ...
        if state then
            WindUI:Notify({ Title="Barista Autofarm", Content="AKTIF!", Duration=3, Icon="coffee" })
        else
...
local logLabel = dbgSection:Paragraph({
    Title = "Log",
    Desc  = "Waiting for output...",
})

dbgSection:Toggle({
    Title    = "Live Log",
    Desc     = "Update log di UI setiap detik",
    Default  = false,
    Flag     = "DebugLiveLog",
...
dbgSection:Button({
    Title    = "Scan Workspace Sekarang",
    Desc     = "Print info BaristaJob & prompts ke console",
    Callback = function()
        dlog("=== MANUAL SCAN ===")
        dlog("Team: " .. (LocalPlayer.Team and LocalPlayer.Team.Name or "NIL"))
        local job  = workspace:FindFirstChild("BaristaJob")
        ...
        if jev then
            local names = ""
            for _, c in ipairs(jev:GetChildren()) do names = names .. c.Name .. " " end
            dlog("JobEvents children: " .. names)
        end
        if job then
            local inter = job:FindFirstChild("Interactions")
            dlog("Interactions: " .. (inter and "ADA" or "TIDAK ADA"))
            if inter then
                for _, c in ipairs(inter:GetChildren()) do
                    dlog("  → " .. c.Name .. " [" .. c.ClassName .. "]")
                    for _, cc in ipairs(c:GetChildren()) do
                        dlog("      → " .. cc.Name .. " [" .. cc.ClassName .. "]")
                        if cc:IsA("ProximityPrompt") then
                            dlog("          ActionText: " .. cc.ActionText)
                            dlog("          Enabled    : " .. tostring(cc.Enabled))
                        end
                    end
                end
            end
        ...
        for _, c in ipairs(workspace:GetDescendants()) do
            if c:IsA("ProximityPrompt") then
                local at = c.ActionText or ""
                if at:lower():find("shift") or at:lower():find("barista") then
                    dlog("PROMPT: " .. c:GetFullName() .. " | " .. at)
                end
            end
        end
        dlog("=== SCAN DONE ===")
        WindUI:Notify({ Title="Debug", Content="Scan selesai — cek console", Duration=3 })
    end,
})
...
task.spawn(function()
    while true do
        ...
        pcall(function()
            local lines = {}
            for i = 1, math.min(8, #debugLog) do
...
function ConfigManager:Save(name)
    if not name or name == "" then
        WindUI:Notify({ Title="Config", Content="Masukkan nama config!", Duration=3 }); return
    end
...
function ConfigManager:Load(name)
    if not name or name == "" then
        WindUI:Notify({ Title="Config", Content="Pilih config dulu!", Duration=3 }); return
    end
    local path = CONFIG_DIR .. name .. CONFIG_EXT
    if not isfile(path) then
        WindUI:Notify({ Title="Config", Content="Config tidak ditemukan!", Duration=3 }); return
    end
...
cfgSection:Input({
    Title="Config Name", Desc="Nama config baru",
    Placeholder="Masukkan nama...", Flag="ConfigNameInput",
...
local configDropdown = loadSection:Dropdown({
    Title="Select Config", Desc="Pilih config tersimpan",
    Values=ConfigManager.AllConfigs, Multi=false, Flag="SelectedConfigDropdown",
...
WindUI:Notify({
    Title="DX-SR Hub", Content="Barista Autofarm v0.0.0.3 loaded! Press V toggle UI.",

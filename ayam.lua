...
local JAWADEOBF_vehicleFly = function(JAWADEOBF_targetCF)
    ...
    while JAWADEOBF_ridegoEnabled do
        ...
        -- berhenti total, tahan di sini
        if JAWADEOBF_dist < 5 then
            JAWADEOBF_bv.Velocity = Vector3.zero
        ...
        -- decelerate 40 stud terakhir supaya tidak nabrak
        local JAWADEOBF_targetSpeed
        if JAWADEOBF_dist < 40 then
            JAWADEOBF_targetSpeed = math.max(18, JAWADEOBF_speed * (JAWADEOBF_dist / 40))
        else
            JAWADEOBF_targetSpeed = JAWADEOBF_speed
        end

        JAWADEOBF_bv.Velocity = JAWADEOBF_delta.Unit * JAWADEOBF_targetSpeed
        task.wait()
    end

    if JAWADEOBF_conn then JAWADEOBF_conn:Disconnect() end

    -- tahan 2 detik sebelum lepas physics â€” model tidak ragdoll/terbang
